...
def firingPower(labels):
    stride = 0
    moving_window_length = 4
    return_list = []
    ...
    if(labels.ndim == 2):
        for j in range(0, labels.shape[1]):
            ...
            while (i < len(col_labels)):
                if (i + moving_window_length < len(col_labels)):
                    fp = np.sum(col_labels[i:i + moving_window_length])
                    fp_list.append(fp)
                    i = i + moving_window_length - stride

                else:
                    fp1 = np.sum(col_labels[i:])
                    fp_list.append(fp1)

            return_list.append(fp_list)
        res = np.array(return_list) / moving_window_length
        ...
        res = np.array(fp_list) / moving_window_length
...
def plotFiringPower(firingPower_labels, firingPower_raw, firingPower_45, firingPower_new, selected_threshold):
    #subplot
    fig, ax = plt.subplots(4, 1, figsize=(10, 10), sharey='col')

    a = firingPower_raw[:, selected_threshold]

    ax[0].plot(a, label='raw')
    ax[0].axhline(y=0.5, color='r', linestyle='--')
    ax[0].set_title('Raw')

    ax[1].plot(firingPower_45[:, selected_threshold], label='45')
    ax[1].axhline(y=0.5, color='r', linestyle='--')
    ax[1].set_title('45')

    ax[2].plot(firingPower_new[:, selected_threshold], label='new')
    ax[2].axhline(y=0.5, color='r', linestyle='--')
    ax[2].set_title('new')

    ax[3].plot(firingPower_labels, label='labels')
    ax[3].axhline(y=0.5, color='r', linestyle='--')
    ax[3].set_title('labels')


    plt.show()

def calculateConfusionMatrixWithFiringPowerLabels(label, firingPower):

    conf_mat_list = []
    for i in range(0, firingPower.shape[1]):
        column = firingPower[:, i]
        fp_label = np.repeat(column, 4)
        fp_label = (fp_label >= 0.5).astype(int)

        fp_label = fp_label.reshape(label.shape[0])

        conf_mat = confusion_matrix(label, fp_label)
...
conf_firingPower_new = []
conf_firingPower_old = []
conf_firingPower_45 = []
...
for p in range(1, 2):
    ...
    for sz in range(1, sz_num + 1):
        ...
        firingPower_old = firingPower(label_raw_data)
        firingPowerOld_list.append(calculateConfusionMatrixWithFiringPowerLabels(label, firingPower_old))
        ...
        firingPower_45 = firingPower(labels_45_data)
        firingPower45_list.append(calculateConfusionMatrixWithFiringPowerLabels(label, firingPower_45))
        ...
        firingPower_new = firingPower(predicted_label)
        firingPowerNew_list.append(calculateConfusionMatrixWithFiringPowerLabels(label, firingPower_new))
        ...
        if(len(selected_threshold_list) != 0):
            ...
            plotFiringPower(firingPower(label),firingPower_old, firingPower_45, firingPower_new, selected_threshold)
    ...
    conf_firingPower_new.append(firingPowerNew_list)
    conf_firingPower_old.append(firingPowerOld_list)
    conf_firingPower_45.append(firingPower45_list)

    #print(len(conf_list_all))


print(len(conf_firingPower_new))
averaged_list = []
